...
Status: **APPROVED**  
Approved: 2026-08-11 by owner (chat: “approve and tell implement agent to implement it and post code to github for review”) with **defaults** from the proposed spec.  
...
Implementation target: `kalshi_mention_scout.py` (current v16 / `VERSION 16.0.0`)  
Stable command: `./mention_scout.py`  
GitHub: https://github.com/rpgrimm/mention-scout/issues/15  
Review publication: **feature branch `openclaw/ms-0010` + pull request into `main`** (do not push straight to `main`)

## Owner decisions (frozen)

1. Match file: JSON `{ "version": 1, "phrases": [...] }` at `~/.config/mention-scout/calendar-matches.json`; seed phrase `abc world news tonight`; repo example under `deploy/config/`.
2. Match semantics: case-insensitive substring; OR across phrases; haystack = parent overview fields + child titles/tickers from snapshot (not rules boilerplate).
3. Reload match file on mtime change without watch restart.
4. Missing/invalid match file with `--calendar-add-new`: **fail fast** at watch start.
5. Watch-only via `--calendar-add-new` (with `--watch-new`).
6. Date-only schedule → **all-day** event on that local date.
7. Missing schedule → no calendar row; failure email once per ticker (dedupe errors).
8. Timed duration: **60** minutes.
9. Calendar id default: `primary`.
10. Optional Google client libs; actionable install hint if missing.
11. Include `--calendar-auth` one-shot; watch never opens a browser.
12. Missing token at start: **fail fast** (run `--calendar-auth`).
13. Missing `client_secret.json`: **fail fast**.
14. Calendar error emails even when `--email-new` is off (SMTP required if calendar on).
15. Reminders: Google defaults.
16. Summary: `[Kalshi] {type_label}: {title}` when type known, else `[Kalshi] {title}`.
17. `--email-new` remains independent of calendar success.
18. Dedupe: `~/.config/mention-scout/calendar-added.json`.
19. **No** `--calendar-types` gate — phrases file alone.
20. Ship path: implement on branch, open PR for review; no merge/ship without later explicit owner approval after independent verification.
21. OAuth secret path: `~/.config/mention-scout/client_secret.json`.
